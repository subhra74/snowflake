...
public abstract class AppSkin {
	...
	private void initDefaults() {
		...
		this.defaults.put("defaultStroke", new BasicStroke(2.0f,
		...
		Painter<? extends JComponent> scrollPaneBorderPainter = (graphics,
		...
		this.defaults.put("ScrollPane[Enabled+Focused].borderPainter",
				scrollPaneBorderPainter);
		this.defaults.put("ScrollPane[Enabled].borderPainter",
				scrollPaneBorderPainter);

		this.defaults.put("ScrollBar.width", 7);

		Painter<? extends JComponent> treeCellFocusPainter = (g, object, width,
				height) -> {
		};
		this.defaults.put("Tree:TreeCell[Enabled+Focused].backgroundPainter",
				treeCellFocusPainter);
	}
	...
	public void createTextFieldSkin(UIDefaults uiDefaults) {
		...
		// Color(230,
		// 230, 230);
		final Color focusedColor = defaults
		...
		Painter<? extends JComponent> painter2 = new Painter<JComponent>() {
			...
			public void paint(Graphics2D g, JComponent object, int width,
				...
				g.setRenderingHint(RenderingHints.KEY_ANTIALIASING,
						RenderingHints.VALUE_ANTIALIAS_ON);
				g.setColor(c1);
				g.fillRoundRect(1 - 20, 1, width - 2 + 20, height - 2 + 20, 5,
						5);
				g.setColor(c2);
				g.drawRoundRect(1 - 20, 1, width - 2 + 20, height - 2 + 20, 5,
						5);
			}
		};
		...
		Painter<? extends JComponent> painter3 = new Painter<JComponent>() {
			...
			public void paint(Graphics2D g, JComponent object, int width,
				...
				g.setRenderingHint(RenderingHints.KEY_ANTIALIASING,
						RenderingHints.VALUE_ANTIALIAS_ON);
				g.setColor(c1);
				g.fillRoundRect(1 - 20, 1 - 20, width - 2 + 20, height - 2 + 20,
						5, 5);
				g.setColor(c2);
				g.drawRoundRect(1 - 20, 1 - 20, width - 2 + 20, height - 2 + 20,
						5, 5);
			}
		};
	...
	public UIDefaults createToolbarSkin() {
		...
		toolBarButtonSkin.put("Button.foreground",
				UIManager.getColor("scrollbar"));
	...
	public void createTableHeaderSkin(UIDefaults uiDefaults) {
		...
		Painter<? extends Object> painterNormal = (Graphics2D g, Object object,
				int width, int height) -> {
//			g.setColor(c1);
//			g.drawLine(width - 1, 3, width - 1, height - 6);
	...
	public void createPopupMenuSkin(UIDefaults uiDefaults) {
		...
		uiDefaults.put("Menu[Enabled+Selected].textForeground",
				selectedTextColor);
		...
		uiDefaults.put("MenuItem:MenuItemAccelerator[Disabled].textForeground",
				textColor);
		uiDefaults.put("MenuItem:MenuItemAccelerator[MouseOver].textForeground",
				textColor);

		Painter<? extends JComponent> popupPainter = (graphics, component,
				width, height) -> {
		...
		Painter<? extends JComponent> menuSelectionPainter = (graphics,
				component, width, height) -> {
			graphics.setColor(
					this.defaults.getColor("nimbusSelectionBackground"));
			graphics.fillRect(0, 0, width, height);
		};

		uiDefaults.put("MenuItem[MouseOver].backgroundPainter",
				menuSelectionPainter);
	}
