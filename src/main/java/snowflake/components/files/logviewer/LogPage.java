...
public class LogPage extends JPanel {
	...
	public LogPage(String filePath, FileComponentHolder holder) {
		...
		txtCurrentPage.setPreferredSize(
				new Dimension(50, txtCurrentPage.getPreferredSize().height));
		txtCurrentPage.setMaximumSize(
				new Dimension(50, txtCurrentPage.getPreferredSize().height));
		txtCurrentPage.setMinimumSize(
				new Dimension(50, txtCurrentPage.getPreferredSize().height));
		...
		logSearchPanel = new PagedLogSearchPanel(new SearchListener() {
			...
			public void search(String text) {
				...
				service.execute(() -> {
					try {
						RandomAccessFile searchIndex = LogPage.this
								.search(text);
...
// }
	...
	private boolean indexFile(boolean xz) {
		try {
			String tempFile = PathUtils.combine(holder.getTempFolder(),
	...
	private String getPageText(long page) throws Exception {
		...
		if (startOffset < 8192) {
			command.append("dd if=\"" + this.filePath + "\" ibs=1 skip="
			...
			command.append("dd if=\"" + this.filePath + "\" ibs=8192 skip="
	...
	private RandomAccessFile search(String text) throws Exception {
		...
		try (OutputStream outputStream = new FileOutputStream(tempFile)) {
			...
			String searchIndexes = PathUtils.combine(holder.getTempFolder(),
					UUID.randomUUID().toString(), File.separator);
