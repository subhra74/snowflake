package snowflake.components.main.settings;
...
public class SettingsPanel extends JPanel {
	private JCheckBox chkConfirmBeforeDelete, chkConfirmBeforeMoveOrCopy,
			...
			chkUseDarkThemeForTerminal, chkShowMessagePrompt,
			chkPuttyLikeCopyPaste, chkUseGlobalDarkTheme;
	...
	public void load(Settings settings) {
		...
		chkConfirmBeforeTerminalClosing
				.setSelected(settings.isConfirmBeforeTerminalClosing());
		chkUseDarkThemeForTerminal
				.setSelected(settings.isUseDarkThemeForTerminal());
		...
		cmbNumberOfSimultaneousConnection
				.setSelectedItem(settings.getNumberOfSimultaneousConnection());
	...
	public void applySettings() {
		...
		settings.setConfirmBeforeTerminalClosing(
				chkConfirmBeforeTerminalClosing.isSelected());
		settings.setUseDarkThemeForTerminal(
				chkUseDarkThemeForTerminal.isSelected());
		...
		settings.setNumberOfSimultaneousConnection(
				(Integer) cmbNumberOfSimultaneousConnection.getSelectedItem());
		...
		try {
			App.applyTheme();
			SwingUtilities.updateComponentTreeUI(App.getMainWindow());
	...
	public SettingsPanel(JFrame frame) {
		...
		chkConfirmBeforeTerminalClosing = new JCheckBox(
				"Confirm before closing a terminal session");
		chkUseDarkThemeForTerminal = new JCheckBox("Use dark terminal theme");
		chkUseGlobalDarkTheme = new JCheckBox("Use global dark theme");
		chkPuttyLikeCopyPaste = new JCheckBox(
				"Use PuTTY like copy paste for terminal");
		chkShowMessagePrompt = new JCheckBox("Show message prompt");

		cmbDefaultOpenAction = new JComboBox<>(new String[] {
				"Open with default application", "Open with default editor",
				"Open with internal editor" });
		cmbDefaultOpenAction
				.setMaximumSize(cmbDefaultOpenAction.getPreferredSize());
		cmbNumberOfSimultaneousConnection = new JComboBox<>(
				new Integer[] { 1, 2, 3, 4, 5, 6, 7 });
		cmbNumberOfSimultaneousConnection.setMaximumSize(
				new Dimension(cmbDefaultOpenAction.getPreferredSize().width * 2,
						cmbDefaultOpenAction.getPreferredSize().height));
		cmbTermType = new JComboBox<>(
				new String[] { "vt100", "xterm", "xterm-256color" });
		cmbTermType.setMaximumSize(
				new Dimension(cmbTermType.getPreferredSize().width * 2,
						cmbTermType.getPreferredSize().height));
		cmbDefaultPanel = new JComboBox<>(Arrays.stream(SnowFlakePanel.values())
				.map(it -> it.getName()).toArray(String[]::new));
		cmbDefaultPanel.setMaximumSize(
				new Dimension(cmbDefaultPanel.getPreferredSize().width * 2,
						cmbDefaultPanel.getPreferredSize().height));
...
//        hb1.add(cmbDefaultOpenAction);
		...
		hb2.add(new JLabel(
				"Number of simultaneous connection for background file transfer"));
