package muonssh.app.ui.components.session.processview;
...
//            btnChangePriority.setEnabled(table.getSelectedRows().length > 0 && !hasPendingOperation.get());
		...
		lblProcessCount = new JLabel(App.bundle.getString("total_processes") +  " 0");
...
//        b1.setBorder(new EmptyBorder(5, 5, 5, 5));
		b1.add(new JLabel(App.bundle.getString("processes")));
		...
		JButton btnFilter = new JButton(App.bundle.getString("filter"));
		...
		JButton btnClearFilter = new JButton(App.bundle.getString("clear"));
		...
		JButton btnRefresh = new JButton(App.bundle.getString("refresh"));
		b1.add(btnRefresh);
		...
		JMenuItem mKill = new JMenuItem(App.bundle.getString("kill"));
		JMenuItem mKillAsRoot = new JMenuItem(App.bundle.getString("kill_sudo"));
		...
		JMenuItem mPrio = new JMenuItem(App.bundle.getString("change_priority"));
		JMenuItem mPrioAsRoot = new JMenuItem(App.bundle.getString("change_priority_sudo"));
		prioPopup.add(mPrio);
...
//        });
		...
		btnCopyArgs = new JButton(App.bundle.getString("copy_command"));
		...
		btnKill = new JButton(App.bundle.getString("kill_process"));
...
//        btnChangePriority.setEnabled(false);
	...
	public void setProcessList(List<ProcessTableEntry> list) {
		lblProcessCount.setText(App.bundle.getString("total_processes") + " " + list.size()
				+ ", "+ App.bundle.getString("last_updated")+" " + LocalDateTime.now().format(
