...
public class SessionInfoPanel extends JPanel {
	...
	private void setJumpHostDetails(boolean useJumpHosts, SessionInfo.JumpType jumpType, List<HopEntry> jumpHosts) {
		this.chkUseJumpHosts.setSelected(useJumpHosts);
		if (jumpType == SessionInfo.JumpType.TcpForwarding) {
	...
	private void updateHopMode() {
		if (radMultiHopPortForwarding.isSelected()) {
			info.setJumpType(SessionInfo.JumpType.PortForwarding);
		} else {
			info.setJumpType(SessionInfo.JumpType.TcpForwarding);
		}
	}
